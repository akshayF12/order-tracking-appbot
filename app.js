...
(function () {
  setTimeout(() => {
    $.ajax({
      url: `https://5893-122-170-106-103.ngrok.io/shop-app-active-status-frontend?shop=${ord_domain}`,
      type: "GET",
      ...
      success: function (data) {
        if (data.app_status == "Disabled") {
          ...
          $("head").append(
            '<link href="https://5893-122-170-106-103.ngrok.io/newapp.css" rel="stylesheet" crossorigin="anonymous">'
          );
          $("body").append(
            '<button class="btn wigets_btn" id="app_btn" onclick="onclickAppbutton();">orderTrack</button>'
          );
          appendApp();
        }
      },
...
function appendApp(open) {
  $(document).ready(function () {
    $.ajax({
      url: "https://5893-122-170-106-103.ngrok.io/wigets-index",
      type: "GET",
      beforeSend: function (xhr) {
        xhr.setRequestHeader("Accept", "*/*");
      },
...
const getRewardData = async () => {
  return new Promise((resolve, reject) => {
    ...
    $.ajax({
      url: "https://5893-122-170-106-103.ngrok.io/get-reward-point",
      type: "GET",
      beforeSend: function (xhr) {
        xhr.setRequestHeader("Accept", "*/*");
      },
...
function checkrewardtPage() {
  ...
  $.ajax({
    url: "https://5893-122-170-106-103.ngrok.io/check-reward-point",
    type: "GET",
    beforeSend: function (xhr) {
      xhr.setRequestHeader("Accept", "*/*");
    },
...
const faqPageData = async () => {
  return new Promise((resolve, reject) => {
    ...
    $("#app_spinner").show();
    $.ajax({
      url: "https://5893-122-170-106-103.ngrok.io/faq-page",
      type: "GET",
      beforeSend: function (xhr) {
        xhr.setRequestHeader("Accept", "*/*");
      },
...
function trackorderPage() {
  ...
  $.ajax({
    url: "https://5893-122-170-106-103.ngrok.io/track-order",
    type: "GET",
    beforeSend: function (xhr) {
      xhr.setRequestHeader("Accept", "*/*");
    },
...
const getcodeData = async (data) => {
  return new Promise((resolve, reject) => {
    ...
    $("#app_index_body").empty();
    $.ajax({
      url: "https://5893-122-170-106-103.ngrok.io/get-code-dicount",
      type: "GET",
      beforeSend: function (xhr) {
        xhr.setRequestHeader("Accept", "*/*");
      },
...
const getfaqdata = (data) => {
  ...
  $.map(faqArray, function (val, i) {
    $(".faq_body").append(
      `<div class="que" id="que${i}" onclick="faqOnclick(${i})"> <h6>${val.question}</h6> <span>${val.answer}</span> </div>`
    );
  });
  $("#app_spinner").hide();
  $("#app_index_body").show();
